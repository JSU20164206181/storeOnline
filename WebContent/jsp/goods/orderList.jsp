			...
			<div class="row margin-bottom-40">
				...
				<div class="col-md-12 col-sm-12">
					<div class="chooseOrder">
						<ul>
							<li id="all"><a
								href="searchAllOrder.action?userId=${cookie.userId.value }"
								style="color: #e84d1c">所有订单</a></li>
							<li id="waitPay"><a
							...
							<li id="waitComment"><a
								href="searchOrderByStatus.action?userId=${cookie.userId.value }&status=4">待评论</a></li>
						</ul>
					</div>
					<div id="shoppingCartNull">
						<div id="shoppingCart" class="goods-page">
										...
										<c:forEach items="${goList}" var="go">
											<tr class="orderInfo">
												...
												<td class="choose" style="position: absolute;right: 20px;"><c:if test="${go.goStatus == 1}">
														<strong><a
															href="payInfo.action?orderId=${go.goId}">立即付款</a></strong>
													</c:if> <c:if test="${go.goStatus == 2}">
														<strong><a href="">订单详情</a></strong>
													</c:if> <c:if test="${go.goStatus == 3}">
														<strong><a href="">订单详情</a></strong>
													</c:if> <br> <strong><a
														href="deleteOrder.action?goid=${go.goId}&status=0">删除订单</a></strong></td>
											</tr>
							...
							<a class="btn btn-primary" id="previousPage"
								href="searchAllOrder.action?userId=${cookie.userId.value }&page=${page+1 }">下一页</a>
							<span class="btn btn-primary">${page } / ${totalPage } </span> <a
								class="btn btn-primary" id="nextPage"
								href="searchAllOrder.action?userId=${cookie.userId.value }&page=${page-1 }">上一页</a>
						</div>
					</div>
				</div>
				<!-- END CONTENT -->
