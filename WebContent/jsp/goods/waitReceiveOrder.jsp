				...
				<div class="col-md-12 col-sm-12">
					<div class="chooseOrder">
						<ul>
							...
							<li id="waitReceive"><a
								href="searchOrderByStatus.action?userId=${cookie.userId.value }&status=3"
								style="color: #e84d1c">待收货</a></li>
							<li id="waitComment"><a
					...
					<div id="shoppingCartNull">
						<div id="shoppingCart" class="goods-page">
										...
										<c:forEach items="${goList}" var="go">
											<tr class="orderInfo">
												...
												<td class="choose" style="position: absolute;right: 20px;"><c:if test="${go.goStatus == 1}">
														...
														<strong><a href="">订单详情</a></strong>
													</c:if> <br> <strong><a href="deleteOrder.action?goid=${go.goId}&status=3">删除订单</a></strong></td>
											</tr>
							...
							<a class="btn btn-primary" id="previousPage"
								href="searchOrderByStatus.action?userId=${cookie.userId.value }&status=3&page=${page+1 }">下一页</a>
							<span class="btn btn-primary">${page } / ${totalPage } </span> <a
								class="btn btn-primary" id="nextPage"
								href="searchOrderByStatus.action?userId=${cookie.userId.value }&status=3&page=${page-1 }">上一页</a>
						</div>
					</div>
				</div>
				<!-- END CONTENT -->
