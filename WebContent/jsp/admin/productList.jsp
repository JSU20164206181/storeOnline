	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
			...
			<tbody>
				...
				<tr class="text-c">
					...
					<td class="td-status">
						<c:if test="${product.gpStatus==1 }">
						<span class="label label-success radius">已上架</span>
						</c:if> 
						<c:if test="${product.gpStatus==0 }">
					...
					<td class="td-manage">
						...
						<c:if test="${product.gpStatus==1 }">
							<a style="text-decoration:none" onClick="product_stop(this,${product.gpId })" href="javascript:;" title="下架"><i class="Hui-iconfont">&#xe6de;</i></a>
						</c:if> 
						<c:if test="${product.gpStatus==0 }">
							<a style="text-decoration:none" onClick="product_start(this,${product.gpId })" href="javascript:;" title="上架"><i class="Hui-iconfont">&#xe603;</i></a>
						</c:if> 
						<a title="编辑" href="javascript:;" onclick="product_edit('编辑','findProductById.action',${product.gpId },'','510')" class="ml-5" style="text-decoration: none"> 
		...
		<div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
			<div class="dataTables_paginate paging_full_numbers" id="tablePage">
				<a class="paginate_button first disabled" aria-controls="DataTables_Table_0" data-dt-idx="0" tabindex="0" id="firstPage" href="findProductList.action?nowPage=1">第一页</a>
				<a class="paginate_button previous disabled" aria-controls="DataTables_Table_0" data-dt-idx="1" tabindex="0" id="previousPage" href="findProductList.action?nowPage=${nowPage-1 }">上一页</a>
				<span><a class="paginate_button current" aria-controls="DataTables_Table_0" data-dt-idx="2" tabindex="0">${nowPage }/${allPage }</a></span>
				<a class="paginate_button next disabled" aria-controls="DataTables_Table_0" data-dt-idx="3" tabindex="0" id="nextPage" href="findProductList.action?nowPage=${nowPage+1 }">下一页</a>
				<a class="paginate_button last disabled" aria-controls="DataTables_Table_0" data-dt-idx="4" tabindex="0" id="lastPage" href="findProductList.action?nowPage=${allPage }">最后一页</a>
			</div>
		</div>
	</div>
</div>
...
function fuzzy_search(){
	if($("#keyword").val()==''){
		window.location.href="findProductList.action";
	}else{
		window.location.href="findProductListByFuzzy.action?keyword="+$("#keyword").val();
	}
...
function product_edit(title,url,id,w,h){
	layer_show(title,url+"?gpId="+id,w,h);
